...
public class QuranSearcher : ISourceSearcher
{
    ...
    public QuranSearcher(ILexicalRetriever retriever)
    {
    ...
    public async Task<IReadOnlyList<KnowledgeMatch>> SearchAsync(SearchContext context, CancellationToken cancellationToken)
    {
        ...
        {
            var doc = await _retriever.GetDocumentByReferenceAsync(qref, cancellationToken);
            if (doc != null)
            {
        ...
        var parameters = new LexicalSearchParameters(
            ...
            TargetSources: new HashSet<EvidenceSource> { EvidenceSource.Quran },
